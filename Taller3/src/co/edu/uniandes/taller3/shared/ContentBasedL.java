...
public class ContentBasedL {
	...
	private ArrayList<Document> getUserDocs(int user, String dateI, String dateF) {
		...
		try{
			...
			int sizeData=9999;
			while(moreRecords>0) {
				String sql = "SELECT movie.movieId, title, genres, info FROM rating INNER JOIN movie ON rating.movieId=movie.movieId WHERE userId=" + user + " timestamp>=UNIX_TIMESTAMP('" + dateI + " 00:00:00') AND timestamp<=UNIX_TIMESTAMP('" + dateF + " 23:59:00') LIMIT " + limit + "," + sizeData + ";";
				System.out.println(sql);
				ResultSet rs = stmt.executeQuery(sql);
				//System.out.println("Cargando desde " + limit);
				limit=limit+sizeData+1;
